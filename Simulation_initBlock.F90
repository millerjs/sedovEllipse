...
#include "Flash.h"
  ...
  integer  ::  i, j, k, n, jLo, jHi, q
  integer  ::  ii, jj, kk
  real     ::  distInv, xDist, yDist, zDist
  real     ::  sumRho, sumP, sumVX, sumVY, sumVZ
  real     ::  vel, diagonal
  real     ::  xx, dxx, yy, dyy, zz, dzz, frac, loop
  real     ::  vx, vy, vz, p, rho, e, ek, maxrho, maxp
  real     ::  dist
  logical  ::  validGeom
  integer :: istat
  ...
  real :: rxy, r, theta, m, a, v, S, ecc

  logical :: gcell = .true.

  real, dimension(10000,4) :: initial
  
  character(20) :: Fname
  Fname = "sedovSolution.dat"
  
  ! Read in the solution file for the elliptical interpolation
  open(unit=10, file=Fname, action="READ")
  do i=1, 17
     read(10,*)
  end do
  do i=1,1000
     read(10,*)(initial(i,j),j=1,4)
  end do
  close(10)
  ...
  do k = blkLimitsGC(LOW,KAXIS), blkLimitsGC(HIGH,KAXIS)
     ...
     do j = blkLimitsGC(LOW, JAXIS), blkLimitsGC(HIGH, JAXIS)
        ...
        do i = blkLimitsGC(LOW,IAXIS), blkLimitsGC(HIGH, IAXIS)
           ...
           do kk = 0, (sim_nSubZones-1)*K3D
              ...
              do jj = 0, (sim_nSubZones-1)*K2D
                 ...
                 do ii = 0, (sim_nSubZones-1)
                    ...
                    ! Old version of sedov, use for non-elliptical shocks
                    ...
                    r = 1.049 ! Set initial radius
                    a = 2     ! Set the aspect ratio of the ellipse

                    ! Set the initial velocity vec to 0
                    vx = 0
                    vy = 0
                    vz = 0
                    ...
                    if (xx < r*a) then
                       theta = sqrt(r**2 - xx**2/a**2)

                       if (yy < theta) then

                          !   Map the space with homeomorphism to 
                          !   an elliptical space with an aspect ratio 1/a.
                          !   We can then assign the mapped space a sedov profile
                          !   based on the angle from the pole and assign to the 
                          !   original (r,theta).  

                          rxy = sqrt( (xx/a)**2 + yy**2)  ! mapped displacement vec
                          theta = sqrt(rxy**2 - xx**2/a**2) 
                          m = a**2/max(xx,1E-4)*theta
                          theta = atan(m) ! mapped angle
                          
                          S = r*( 1 + ecc * cos(theta)**2)**.5
                          rxy = rxy/S
                          
                          ! Find value in solution table
                          q = 1
                          do while (q < 1000 .and. rxy > initial(q,1))
                             q = q+1
                          end do
                          ...
                          if (q <= 2) then
                             ...
                             m = (initial(q,2)-initial(q-1,2))/(initial(q,1)-initial(q-1,1))
                             v = m * (r - initial(q,1)) + initial(q,2)

                             m = (initial(q,3)-initial(q-1,3))/(initial(q,1)-initial(q-1,1))
                             rho = m * (r - initial(q,1)) + initial(q,3) 

                             m = (initial(q,4)-initial(q-1,4))/(initial(q,1)-initial(q-1,1))
                             p = m * (r - initial(q,1)) + initial(q,4)
                    ...
                    ! vel = v
                    ! sim_vProf(jLo) + frac*(sim_vProf(jHi)  - sim_vProf(jLo))
           ...
           ! rho = max(sumRho * sim_inszd, sim_smallRho*1.1)
           ! p   = max(sumP   * sim_inszd, sim_smallP*1.1)
           ! vx  = sumVX  * sim_inszd
           ! vy  = sumVY  * sim_inszd
           ! vz  = 0

           rho = max(sumRho/loop, sim_smallRho*1.1)
           p   = max(sumP/loop  , sim_smallP*1.1)
